...
class TeamService:
    @classmethod
    async def create_team(cls, team_create: TeamCreateDTO) -> TeamModel:
        team_exist = TeamExistDTO(**team_create.__dict__)
    ...
    @classmethod
    async def check_team_exists(cls, team_exist: TeamExistDTO) -> bool:
        exists = await TeamRepo.check_team_exists_by_workspace_id_and_user_id(team_exist=team_exist)
        if not exists:
