...
class AuthService:
    ...
    async def create_access_token(cls, form_data: OAuth2PasswordRequestForm) -> Token:
        ...
        access_token = jwt.encode(to_encode, Settings().SECRET_KEY, algorithm=cls.ALGORITHM)
    ...
    def get_username_from_token(cls, token: str) -> str:
        try:
            decoded_jwt = jwt.decode(token, Settings().SECRET_KEY, algorithms=[cls.ALGORITHM])
