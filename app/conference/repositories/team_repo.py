...
class TeamRepo:
    @classmethod
    async def check_team_exists_by_workspace_id_and_user_id(cls, team_exist: TeamExist) -> bool:
    ...
    @classmethod
    async def create(cls, team_create: TeamCreate) -> TeamModel:
        team_model = await TeamModel.objects.create(**team_create.__dict__)
        return team_model

    @classmethod
    async def get_team_by_workspace_id_and_user_id(cls, team_exist: TeamExist):
        team_model = await TeamModel.objects.get(workspace_id=team_exist.workspace_id, user_id=team_exist.user_id)
        return team_model

    @classmethod
    async def accept(cls, team_accept: TeamAccept) -> TeamModel:
    ...
    @classmethod
    async def reject(cls, team_accept: TeamAccept) -> TeamModel:
        team_model = await TeamModel.objects.get(workspace_id=team_accept.workspace_id, user_id=team_accept.user_id)
