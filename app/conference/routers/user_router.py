...
from conference.schemas.user_schema import UserCreateDTO, UserResetPasswordDTO

router = APIRouter()


@router.post("/register", response_model=UserModel)
async def register(user_in: UserCreateDTO = Body(...)) -> UserModel:
    return await UserService.create_user(user_in=user_in)


@router.post("/reset-password", response_model=UserModel)
async def reset_password(user: UserResetPasswordDTO = Body(...)) -> UserModel:
